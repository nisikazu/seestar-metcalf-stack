...
## 2026-08-22: north-up PNG の回転符号修正（v0.8.1）

- FITS WCSの逆CD行列から、天の北 `(dRA, dDec) = (0, 1)` が元画像で向くベクトルを得る。通常プレビューはFITS座標のY軸とPillow画像座標のY軸が逆向きのため、表示ベクトルのY成分を反転する。
- Pillowの`Image.rotate(+angle)`は画面上で反時計回りに回転する。そのため、表示座標での北ベクトル角を`current`とすると、北を上 `-90°` に置くために渡す回転角は`current + 90°`である。v0.8.0はこの符号を反転していた。
- 220P McNaughtの実WCS（CD行列）を回帰テストへ追加した。標準north-up WCSだけでは回転量が0になり符号誤りを検出できないため、必ず斜め向きのWCSでも検証する。
