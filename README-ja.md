...
- **Node.js** はAstrometry.netへのアップロードと完了待ちを行う小さな補助プログラムに使います。
...
GitHub本体と標準版 `seestar-metcalf-stack-vX.Y.Z.zip` はSirilを同梱しません。Sirilを別途インストールし、`siril-cli.exe` にPATHを通すか、環境変数 `SIRIL_CLI` にフルパスを設定します。

容量の大きい `seestar-metcalf-stack-siril-vX.Y.Z.zip` はSiril同梱Windows便利版です。準備を簡単にしたい場合はこちらを使えます。同梱されるSiril部分にはGPLv3が適用されます。

## 初回セットアップ

1. PythonとNode.jsをインストールします。標準版ではSirilもインストールします。
2. 展開したフォルダでPython依存パッケージを準備します。
...
サブフレームフォルダを `run-metcalf-stack-drop.cmd` にドラッグ&ドロップしても実行できます。成功すると出力フォルダが開きます。
