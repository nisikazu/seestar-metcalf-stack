...
容量の大きい `seestar-metcalf-stack-siril-vX.Y.Z.zip` は、SirilとPython実行環境を含むWindows便利版です。`seestar-metcalf-stack.exe`を使うためPythonを別途インストールする必要はありません。同梱されるSiril部分にはGPLv3が適用されます。

## 初回セットアップ

1. Pythonをインストールします。標準版ではSirilもインストールします。
...
サブフレームフォルダを `seestar-metcalf-stack-drop.cmd` にドラッグ&ドロップしても実行できます。成功すると出力フォルダが開きます。
