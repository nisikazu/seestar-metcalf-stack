...
容量の大きい `seestar-metcalf-stack-siril-vX.Y.Z.zip` は、SirilとPython実行環境を含むWindows便利版です。`metcalf-stack.exe`を使うためPythonを別途インストールする必要はありません。同梱されるSiril部分にはGPLv3が適用されます。
