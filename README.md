...
`offset`は従来どおり中央70%の有効画素からRGBごとのsigma-clipped medianを求め、DC成分だけを揃えます。視野内の空の明るさに傾きがある場合は、`plane`または`quadratic`を選べます。これらは実画像の有効画素を50x50タイルに分け、各タイルを4画素間隔でサンプリングしたsigma-clipped medianを使って一次平面または二次曲面をフィットします。一度フィットした残差のMADから外れるタイルを除外して一回だけ再フィットするため、星や小さな彗星の影響を抑えながら、毎回同じ結果になります。
