...
数百枚のセッションでは、Sirilの変換・補正・デベイヤ中に元FITS、変換像、前処理済み画像が一時的に共存するため、元FITSの合計より大きな空き容量が必要です。通常のSiril背景星登録は`register -2pass`で行列だけを取得し、登録済みFITSは生成しません。Sirilが `Not enough free disk space` を表示した場合は、空き容量を増やす、`--work-root D:\metcalf_output` のように別ドライブを使う、または `--count 400` のように処理枚数を減らしてください。前処理成功後はsource copy・変換像・staged dark/flatを、登録行列の取得後は不要な前処理済み画像を、各フレームの寄与確定後は残りのスタック入力を順次削除します。完全なSharpCap StackLogを使う経路では、記録された変換を適用した登録画像を従来どおり一時生成します。中間画像を調査用に残す場合は`--no-cleanup`を指定します。
...
画素ごとのメジアンは、人工衛星、飛行機、ホットピクセルなど少数フレームだけに現れる外れ値に強い方式です。メジアンはメトカーフスタッキング像において星の軌跡を低減し、彗星光度の精度向上を図ります。一方で平均より遅く、統計的な効率も通常は平均より低くなります。メジアンでは登録・シフト境界に現れる完全な0をデフォルトで母集団から除外します。
...
`--median-tile-rows`の値は**分割数ではなく、1回に処理する行数（縦方向のピクセル数）**です。各タイルは画像の横幅全体を持ちます。例えば高さ1920ピクセルの画像で`--median-tile-rows 720`を指定すると、720行ずつ、合計3タイルで処理します。速度やメモリ使用量を比較する場合は、1タイルの行数を明示できます。値を小さくするとピークRAMは減りますが、各サブフレームを読む回数が増えるため通常は遅くなります。
...
出力範囲は`--output-region`で指定します。指定値は`reference`、`union`、整数の枚数、または`%`付きの割合です。`reference`は既定値で、従来どおり基準フレームと同じ範囲・大きさを出力します。位置合わせ後の採用フレームがその外側まで持っている画素も残すには、`union`を指定します。

```bat
.\seestar-metcalf-stack.cmd "C:\path\to\frames" --output-region union
```

重なりの少ない周辺を除き、指定枚数以上が重なる範囲の外接矩形を出力するには整数を指定します。ここで数えるのは、雲などで登録に失敗して除外された画像を除く、実際の採用フレームです。

```bat
.\seestar-metcalf-stack.cmd "C:\path\to\frames" --output-region 20
```

採用フレームの指定割合以上が重なる範囲には`%`付きの値を指定します。小数も指定できます。必要枚数は切り上げます。例えば採用53枚の75.0%は40枚です。
...
拡張領域は画像サイズ、RAM、処理時間、出力容量を増やします。メジアンとランクフィットでは既存の行タイル処理が拡張領域にも適用されます。`union`または整数・割合指定の`--output-region`は、実画素だけを数える`--padding-policy valid`専用です。SharpCap StackLogの位置合わせ経路は前処理中に基準範囲へ切り出した一時画像を使うため、現時点では`reference`だけを使用できます。
...
source copy・変換像・staged calibrationは前処理成功後に削除します。通常のSiril経路では`register -2pass`の行列取得後、登録済みFITSを作りません。平均では前処理済み画像を各フレームの寄与確定後に削除します。メジアン/ランクフィットでは行タイルごとに再読込するため全タイル完了まで前処理済み画像を保持し、完了後に削除します。全画面のメジアンキューブを一時ファイルとして保存することはありません。調査のため残す場合は `--no-cleanup` を指定します。
