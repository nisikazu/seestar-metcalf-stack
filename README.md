...
source copy・変換像・staged calibrationは前処理成功後に削除します。通常のSiril経路では`register -2pass`の行列取得後、前処理済み画像を各フレームのスタック反映後に削除し、登録済みFITSは作りません。SharpCap StackLog経路で生成する登録画像は各フレームの反映後、メジアン用一時配列は最終化後に削除します。調査のため残す場合は `--no-cleanup` を指定します。
